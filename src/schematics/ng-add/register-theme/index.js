...
function registerPrebuiltTheme(options) {
    return (0, workspace_1.updateWorkspace)((workspace) => __awaiter(this, void 0, void 0, function* () {
        ...
        const themePath = `./node_modules/@josedcx/ang-theme/src/styles/prebuilt/${options.theme}.css`;
        addStyleToTarget(project, 'build', themePath);
    }));
}
exports.registerPrebuiltTheme = registerPrebuiltTheme;
/**
 * Creates theme.scss with Ang theme setup.
 * */
...
function addStyleToTarget(project, targetName, stylesPath) {
    ...
    else if (noAngThemeIncluded(targetOptions, stylesPath)) {
        targetOptions.styles.unshift(stylesPath);
    }
}
/**
 * Validates no Ang styles already included into the specified project.
 * */
function noAngThemeIncluded(targetOptions, stylesPath) {
    const existingStyles = targetOptions.styles.map((s) => typeof s === 'string' ? s : s.input);
    const hasGivenTheme = existingStyles.find((s) => s.includes(stylesPath));
    const hasOtherTheme = existingStyles.find((s) => s.includes('@josedcx/ang-theme/src/styles/prebuilt'));
